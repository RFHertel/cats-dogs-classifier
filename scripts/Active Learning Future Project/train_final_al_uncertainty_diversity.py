...
Uses margin-based uncertainty with k-means clustering for diversity.

Usage:
    python scripts/train_final_al_uncertainty_diversity.py
...
def select_with_diversity(uncertainties, features, n_select, pool_multiplier=2):
    """
    Select diverse samples from the most uncertain ones.
    
    1. Take top (n_select * pool_multiplier) most uncertain
    2. Cluster using k-means
    3. From each cluster, pick the most uncertain
    """
    candidate_size = min(n_select * pool_multiplier, len(uncertainties))
    candidate_indices = np.argsort(uncertainties)[::-1][:candidate_size]
    ...
    for cluster_id in range(n_clusters):
        ...
        cluster_uncertainties = candidate_uncertainties[cluster_local_indices]
        best_in_cluster = cluster_local_indices[np.argmax(cluster_uncertainties)]
...
def run_active_learning(train_dataset, val_loader, config):
    """Run active learning with uncertainty + diversity sampling."""
    print(f"\n{'='*60}")
    print(f"Strategy: UNCERTAINTY + DIVERSITY")
    print(f"{'='*60}")
    ...
    for round_num in range(config['max_rounds']):
        ...
        # Compute uncertainty and features
        print("  Computing uncertainty and features...")
