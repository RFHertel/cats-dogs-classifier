...
Usage:
    python scripts/train_final_al_entropy.py
...
config = {
    ...
    'output_dir': Path('outputs/final_active_learning_entropy'),  # Separate folder
...
def compute_entropy(model, dataloader, device):
    ...
    For binary: max entropy at p=0.5 is log(2) ≈ 0.693
    """
    model.eval()
    entropies = []
...
def run_active_learning(train_dataset, val_loader, config):
    ...
    for round_num in range(config['max_rounds']):
        ...
        entropy_order = np.argsort(entropies)[::-1]
        selected_local = entropy_order[:n_to_add]
        selected_indices = [unlabeled_indices[i] for i in selected_local]
        
        # Clear memory
        del model, unlabeled_loader, entropies
