"""
Active Learning - Uncertainty Sampling (Margin-based)

Usage:
    python scripts/train_final_al_uncertainty.py
...
config = {
    ...
    'output_dir': Path('outputs/final_active_learning'),
...
def compute_uncertainty_margin(model, dataloader, device):
    """
    Margin sampling: uncertainty = 1 - |p_class0 - p_class1|
    Higher score = more uncertain (closer to 0.5 probability)
    """
    model.eval()
    uncertainties = []
...
def run_active_learning(train_dataset, val_loader, config):
    ...
    print(f"\n{'='*60}")
    print(f"Strategy: UNCERTAINTY (Margin Sampling)")
    print(f"{'='*60}")
    ...
    results = []
    final_model = None
    least_certain_tracking = []  # Track top 50 uncertain images per round
    ...
    for round_num in range(config['max_rounds']):
        ...
        uncertainties = compute_uncertainty_margin(model, unlabeled_loader, config['device'])
        
        # Get top 50 most uncertain for tracking
        top50_local = np.argsort(uncertainties)[::-1][:50]
        top50_uncertain = []
        for idx in top50_local:
            global_idx = unlabeled_indices[idx]
            top50_uncertain.append({
                'filename': train_dataset.file_list[global_idx],
                'uncertainty': float(uncertainties[idx]),
            })
        
        # Add to tracking and save immediately
        least_certain_tracking.append({
            'round': round_num + 1,
            'labeled_size': len(labeled_indices),
            'top50_uncertain': top50_uncertain,
        })
        
        least_certain_file = config['output_dir'] / f'least_certain_images_{STRATEGY}.json'
        with open(least_certain_file, 'w') as f:
            json.dump(least_certain_tracking, f, indent=2)
        
        # Select most uncertain
        n_to_add = min(config['samples_per_round'], len(unlabeled_indices))
        uncertain_order = np.argsort(uncertainties)[::-1]
        selected_local = uncertain_order[:n_to_add]
        selected_indices = [unlabeled_indices[i] for i in selected_local]
        
        # Clear memory
        del model, unlabeled_loader, uncertainties
        torch.cuda.empty_cache()
        
        labeled_indices.extend(selected_indices)
        unlabeled_indices = [i for i in unlabeled_indices if i not in set(selected_indices)]
    
    return results, best_overall_model, best_round, best_overall_acc
...
def main():
    ...
    results, best_model_state, best_round, best_acc = run_active_learning(train_dataset, val_loader, config)
    ...
    print(f"\nSaved best model (round {best_round}, acc={best_acc:.4f}): {model_path}")
    ...
    print(f"Best round: {best_round} (labeled={results[best_round-1]['labeled_size']}, acc={best_acc:.4f})")
