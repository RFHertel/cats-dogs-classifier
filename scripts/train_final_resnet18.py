"""
Final ResNet18 model training on full dataset.

Uses best hyperparameters from MobileNetV2 experiments:
...
config = {
    ...
    'model_dir': Path('outputs/final_resnet18'),  # Separate from MobileNetV2
    'output_dir': Path('outputs/final_resnet18'),
    
    # Best hyperparameters from MobileNetV2 experiments
...
def create_resnet18(num_classes=2, pretrained=True):
    """Create ResNet18 with pretrained weights."""
    ...
    # ResNet18's fc layer takes 512 features
...
def main():
    print("=" * 60)
    print("FINAL RESNET18 MODEL TRAINING")
    print("=" * 60)
    ...
    print(f"Model: ResNet18 ({param_count:,} parameters)")
    print()
    ...
    # Rename best model to final
    import shutil
    final_model_path = config['output_dir'] / 'final_resnet18_model.pth'
    ...
    print(f"  - final_resnet18_model.pth")
